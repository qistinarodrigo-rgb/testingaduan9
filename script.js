...
// ========== SAVE TO FIREBASE (guna Storage untuk gambar) ==========
async function saveAduanToFirebase(aduan, gambarFile) {
    try {
        let gambarUrl = null;
        
        // Kalau ada gambar, upload ke Storage dulu
        if (gambarFile) {
        ...
        // Simpan data aduan dengan URL gambar (bukan base64)
        const newAduan = {
            ...aduan,
            gambar: gambarUrl
        };
...
async function deleteAduanFromFirebase(id) {
    try {
        // Dapatkan data aduan dulu
        const aduanDoc = await db.collection('aduan').doc(id).get();
        const aduanData = aduanDoc.data();
        
        // Kalau ada gambar, delete dari Storage
        if (aduanData.gambar && aduanData.gambar.includes('firebasestorage')) {
            try {
                const storageRef = storage.refFromURL(aduanData.gambar);
        ...
        html += `
            <div class="card">
                ...
                <form id="formAduan" onsubmit="return false;">
                    ...
                    <button type="button" class="btn-primary" id="hantarAduanBtn">
...
// ========== HANDLE HANTAR ADUAN (VERSI DEBUG IPHONE) ==========
async function handleHantarAduan() {
    const submitBtn = document.getElementById('hantarAduanBtn');
    const originalText = submitBtn.innerHTML;
    submitBtn.innerHTML = '⏳ Menghantar...';
    submitBtn.disabled = true;
    
    try {
        // ALERT 1: Check storage
        alert('1. Storage check: ' + (typeof storage !== 'undefined' ? 'OK' : 'Takde'));
        
        const gambarFile = document.getElementById('gambar').files[0];
        
        // ALERT 2: Check gambar
        alert('2. Gambar: ' + (gambarFile ? gambarFile.name + ' (' + (gambarFile.size/1024).toFixed(2) + 'KB)' : 'Tiada'));
        
        // Check saiz gambar (5MB max)
        if (gambarFile && gambarFile.size > 5 * 1024 * 1024) {
            alert('❌ Gambar terlalu besar! Maksimum 5MB');
        ...
        // ALERT 3: Data sedia
        alert('3. Data sedia dihantar');

        // Simpan ke Firebase (dengan gambar kalau ada)
        await saveAduanToFirebase(aduanData, gambarFile);
        
        // ALERT 4: Lepas save
        alert('4. Berjaya simpan ke Firebase');
        
        // Refresh data
        aduanList = await loadAduan();
        const userType = localStorage.getItem('userType');
        await refreshSenarai(userType);
        ...
        showNotification('Gagal hantar aduan. Sila cuba lagi.', false);
    } finally {
        submitBtn.innerHTML = originalText;
        submitBtn.disabled = false;
    }
